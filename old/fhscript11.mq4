...
int start() {
   ...
   Comment( 
            ...
            //"\ndbliBandsMid: " + DoubleToStr(dbliBandsMid, 2) +
            "\ndbliMFI: " + DoubleToStr(dbliMFI, 2)
            );
   ...
   for (intCurrentOrder = 0; intCurrentOrder < OrdersTotal(); intCurrentOrder++) {
     ...
     if ( ( (OrderType() == OP_BUY) && (OrderSymbol() == Symbol()) ) && (dbliMFI <= 50) || (OrderProfit() >= 3)  ) { //((dblPriceDiff <= 0) && ((OrderProfit() > 0) && (OrderProfit() > dblLastProfit)) ) ) {
         ticketDell = OrderClose(OrderTicket(), OrderLots(), MarketInfo(OrderSymbol(), MODE_BID), 3, Silver);
         Comment( "\nfechar! >= buy");
         Sleep(300000); //5 min
      }

      if ( ( (OrderType() == OP_SELL) && (OrderSymbol() == Symbol())) && (dbliMFI >= 50) || (OrderProfit() >= 3) ) { // && ((dblPriceDiff >= 0) && ((OrderProfit() > 0) && (OrderProfit() < dblLastProfit)) ) ) {
